...
class NetlistReader:
    ...
    def _parse_component(self, comp_block: str) -> Optional[Component]:
        ...
        if ref and value and footprint:
            footprint = normalize_footprint_name(footprint)
    ...
    def _parse_net(self, net_block: str) -> Optional[Net]:
        ...
        if code is not None and name is not None:
            ...
            node_blocks = self._extract_blocks(net_block, "node")
            for node_block in node_blocks:
                ref = self._extract_field(node_block, "ref")
                pin = self._extract_field(node_block, "pin")
                if ref and pin:
