...
def get_footprint_pads(footprint_name: str) -> tuple[list[Pad], str | None]:
    ...
    Args:
        footprint_name: KiCad footprint library name
    ...
    if ":" in footprint_name:
        footprint_library, footprint_type = footprint_name.split(":", 1)
    else:
        footprint_type = footprint_name
    is_kicad_pin_header = footprint_library == "Connector_PinHeader_2.54mm"
    ...
    handlers = {
        ...
        "C_0805": _c0805,
        "C_0603_1608Metric": _c0603,
        "C_0603": _c0603,
        "LED_0603_1608Metric": _led0603,
        # ICs - SMD
        "SOT-223-3": _sot223_3,
        ...
        "Fiducial_1mm_Mask2mm": _fiducial_1mm,
        "MountingHole_3.2mm_M3": _mounting_hole_3_2mm,
        # Connectors
        "PinHeader_1x03_P2.54mm_Vertical": lambda: _pin_header_1xn(
            3, is_kicad_pin_header
        ),
        "PinHeader_1x04_P2.54mm_Vertical": lambda: _pin_header_1xn(
            4, is_kicad_pin_header
        ),
        "PinHeader_1x04_P2.54mm": lambda: _pin_header_1xn(4, is_kicad_pin_header),
        "PinHeader_1x05_P2.54mm_Vertical": lambda: _pin_header_1xn(
            5, is_kicad_pin_header
        ),
        "PinHeader_1x06_P2.54mm_Vertical": lambda: _pin_header_1xn(
            6, is_kicad_pin_header
        ),
        "PinHeader_1x07_P2.54mm_Vertical": lambda: _pin_header_1xn(
            7, is_kicad_pin_header
        ),
        "PinHeader_1x08_P2.54mm_Vertical": lambda: _pin_header_1xn(
            8, is_kicad_pin_header
        ),
        "PinHeader_1x10_P2.54mm_Vertical": lambda: _pin_header_1xn(
            10, is_kicad_pin_header
        ),
    }
    ...
    # Step 3: Try local fixture footprints when compile-physical runs from a
    # self-contained atopile export directory.
    pads = _try_local_kicad_mod_load(footprint_type)
    if pads:
        return (pads, None)

    # Step 4: Try the template library for package families that are generated
    # elsewhere in the toolchain but not listed in this legacy handler table.
    try:
        from pardal.footprint_templates import generate_pads
    except ImportError:
        generate_pads = None
    if generate_pads is not None:
        try:
            return (generate_pads(footprint_type), None)
        except ValueError:
            pass

    # Step 5: Try pcbnew SDK fallback
    pads = _try_pcbnew_load(footprint_name)
    if pads:
        return (pads, None)

    # Step 6: Fail with error (NOT single pad!)
...
def _c0603() -> list[Pad]:
    """0603 capacitor."""
    return [
        Pad(number=1, position_offset=(-0.775, 0.0), size=(0.9, 0.95), shape="rect"),
        Pad(number=2, position_offset=(0.775, 0.0), size=(0.9, 0.95), shape="rect"),
    ]


def _led0603() -> list[Pad]:
    """0603 LED."""
    return [
        Pad(number=1, position_offset=(-0.7875, 0.0), size=(0.875, 0.95), shape="rect"),
        Pad(number=2, position_offset=(0.7875, 0.0), size=(0.875, 0.95), shape="rect"),
    ]
...
def _sot223_3() -> list[Pad]:
    ...
    return [
        Pad(
            number=1, position_offset=(-2.3, 3.2), size=(1.0, 2.0), shape="rect"
        ),  # Input
        Pad(
            number=2, position_offset=(0.0, 3.2), size=(1.0, 2.0), shape="rect"
        ),  # GND/Tab
        Pad(
            number=3, position_offset=(2.3, 3.2), size=(1.0, 2.0), shape="rect"
        ),  # Output
        Pad(
            number=4, position_offset=(0.0, -3.2), size=(3.5, 2.0), shape="rect"
        ),  # Tab/heat sink
    ]
...
def _ssop20() -> list[Pad]:
    ...
    for i in range(10):
        ...
        pads.append(
            Pad(
                number=i + 1,
                position_offset=(-3.9, -2.925 + i * 0.65),
                size=(1.6, 0.4),
                shape=shape,
            )
        )
...
def _dip8() -> list[Pad]:
    ...
    for i in range(4):
        ...
        pads.append(
            Pad(
                number=i + 1,
                position_offset=(-3.81, -3.81 + i * 2.54),
                size=(1.6, 1.6),
                drill=0.8,
                shape=shape,
            )
...
def _fiducial_1mm() -> list[Pad]:
    return [Pad(number=1, position_offset=(0.0, 0.0), size=(1.0, 1.0), shape="circle")]


def _mounting_hole_3_2mm() -> list[Pad]:
    return [
        Pad(
            number=1,
            position_offset=(0.0, 0.0),
...
def _pin_header_1xn(num_pins: int, origin_at_pin1: bool = False) -> list[Pad]:
    ...
    Returns:
        List of pads spaced 2.54mm apart. KiCad library footprints use pad 1
        as the origin; legacy unqualified names remain centered.
    """
    pads = []
    center_offset = 0.0 if origin_at_pin1 else (num_pins - 1) * 2.54 / 2
...
def _try_local_kicad_mod_load(footprint_type: str) -> list[Pad] | None:
    """Load pads from local .kicad_mod files bundled beside fixture specs."""
    candidates = [
        root / f"{footprint_type}.kicad_mod" for root in _local_footprint_roots()
    ]
    for path in candidates:
        if not path.is_file():
            continue
        pads = _extract_pads_from_kicad_mod_text(path.read_text(encoding="utf-8"))
        if pads:
            return pads
    return None


def _local_footprint_roots() -> list[Path]:
    roots: list[Path] = []
    env_roots = os.environ.get("PARDAL_LOCAL_FOOTPRINT_DIRS", "")
...
def _extract_pads_from_kicad_mod_text(text: str) -> list[Pad]:
    pads: list[Pad] = []
    pad_starts = [match.start() for match in re.finditer(r"\(\s*pad\s+", text)]
    for index, start in enumerate(pad_starts):
        end = pad_starts[index + 1] if index + 1 < len(pad_starts) else len(text)
        block = text[start:end]
        header = re.search(r'\(\s*pad\s+"?([^"\s)]+)"?\s+(\S+)\s+(\S+)', block)
        at = re.search(r"\(\s*at\s+([-0-9.]+)\s+([-0-9.]+)(?:\s+[-0-9.]+)?\s*\)", block)
        size = re.search(r"\(\s*size\s+([-0-9.]+)\s+([-0-9.]+)\s*\)", block)
        if not (header and at and size):
            continue
        number, pad_type, shape = header.groups()
        x, y = at.groups()
        sx, sy = size.groups()
        drill = re.search(r"\(\s*drill\s+([-0-9.]+)(?:\s+[-0-9.]+)?\s*\)", block)
        drill_size = float(drill.group(1)) if drill else None
        if pad_type in {"thru_hole", "np_thru_hole"} and drill_size is None:
            return []
        pads.append(
            Pad(
                number=number,
                ...
                drill=drill_size,
                shape=shape if shape in {"circle", "rect", "oval"} else "rect",
            )
        )
    return pads
