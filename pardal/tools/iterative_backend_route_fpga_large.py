...
def _apply_supports_clear_nets(*, repo_root: Path, image: str) -> bool:
    ...
    cmd = [
        ...
        image,
        "python3",
        "pardal-pcb/pardal/tools/apply_routes_pcbnew.py",
        "--help",
    ]
    try:
...
def _extract_problem(
    *,
    ...
    cmd = [
        ...
        image,
        "python3",
        "pardal-pcb/pardal/tools/extract_routing_problem_pcbnew.py",
        "--pcb",
        f"/work/{in_pcb}",
        "--out",
        f"/work/{out_problem}",
...
def _clear_window(
    *,
    ...
    cmd = [
        ...
        image,
        "python3",
        "pardal-pcb/pardal/tools/clear_window_pcbnew.py",
        "--pcb",
        f"/work/{in_pcb}",
        "--out",
        f"/work/{out_pcb}",
...
def _clear_uuids(
    *,
    ...
    cmd = [
        sys.executable,
        "pardal-pcb/pardal/tools/clear_uuids_text.py",
        "--pcb",
        str(in_pcb),
        "--out",
        str(out_pcb),
...
def main() -> int:
    ...
    router_bin = repo_root / "pardal-pcb" / "routing" / "mojo_router" / "build" / "pardal-router-mojo"
    if not router_bin.exists():
        _run([str(repo_root / "pardal-pcb" / "routing" / "mojo_router" / "build.sh")], cwd=repo_root / "pardal-pcb" / "routing" / "mojo_router")
    if not router_bin.exists():
