...
from pardal.freerouting_backend import FreeroutingRunConfig, freeroute_kicad_pcb, sanitize_dsn
...
def _freerouting_build_dir(repo_root: Path) -> Path:
    # Matches `pardal.freerouting_backend._build_dir()` (pardal-pcb/build/freerouting).
...
def _mojo_dsn_dump(
    *,
    ...
    router_bin = repo_root / "routing" / "mojo_router" / "build" / "pardal-router-mojo"
    if not router_bin.exists():
        build_script = repo_root / "routing" / "mojo_router" / "build.sh"
