...
This wraps:
- Internal simplified DRC (`pardal.commands.drc.CheckDrcCommand`) for fast feedback.
- KiCad DRC via `kicad-cli` (`pardal.drc.run_drc`) for production validation.
- KiCad DRC via `pcbnew` SDK (`pardal.drc.run_sdk_drc`) when available.
"""
