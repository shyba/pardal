...
FAST_FIXTURES = [
    "freerouting/tests/Issue269-min_fr_test/min_fr_test.kicad_pcb",
    "freerouting/tests/Issue180-Test/Test.kicad_pcb",
    "freerouting/tests/Issue269-NoViasOnPowerPlanes/Issue269-NoViasOnPowerPlanes.kicad_pcb",
]
...
def _run_check(*, fixture: Path, out_dir: Path, timeout_s: float) -> dict:
    ...
    proc = subprocess.run(
        ...
        capture_output=True,
        text=True,
        timeout=max(int(timeout_s) + 120, 180),
    )
...
@pytest.mark.slow
@pytest.mark.parametrize("fixture_rel", FAST_FIXTURES)
def test_mojo_matches_fr_baseline_fast(fixture_rel: str, tmp_path: Path) -> None:
    ...
    out_dir = _repo_root() / "build" / "pytest_parity_baseline" / tmp_path.name / "fast"
    out_dir.mkdir(parents=True, exist_ok=True)
    _run_check(fixture=fixture, out_dir=out_dir, timeout_s=120.0)
