...
@pytest.mark.slow
def test_mojo_precommit_keepout_other_layer_does_not_block(tmp_path: Path):
    pixi = shutil.which("pixi") or str(Path.home() / ".pixi" / "bin" / "pixi")
    if not pixi or not Path(pixi).exists():
        pytest.skip("pixi not available; skipping Mojo keepout layer-scope test")
    ...
    # Keepout exists only on B.Cu but routing happens on F.Cu only; precommit should not block.
    problem.write_text(
        json.dumps(
            {
                "source": "synthetic",
        ...
        json.dumps(
            {
                ...
                # Force routing to stay on F.Cu.
                "net_layer_allow": {"N1": ["F.Cu"]},
            },
            indent=2,
        )
    )
    ...
    assert "N1" not in out.get("failed_nets", [])
