    ...
    def test_all_mappings_have_valid_lib_names(self):
        ...
        for fp_name, lib_path in FOOTPRINT_LIBS.items():
            assert lib_path.endswith(
                ".pretty"
            ), f"Invalid lib path for {fp_name}: {lib_path}"
            assert (
                "/usr/share/kicad/footprints/" in lib_path
            ), f"Non-standard lib path for {fp_name}"
    ...
    def test_list_shows_pad_count(self):
        ...
        comp = Component(
            ...
            pads=[
                Pad(
                    number="1",
                    position_offset=(-0.95, 0),
                    size=(1.0, 1.3),
                    shape="rect",
                ),
                Pad(
                    number="2", position_offset=(0.95, 0), size=(1.0, 1.3), shape="rect"
                ),
            ],
        )
        board.add_component(comp)
