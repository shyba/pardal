...
@pytest.mark.slow
def test_dsn_to_problem_to_mojo_routes_smoke(tmp_path: Path):
    pixi = shutil.which("pixi") or str(Path.home() / ".pixi" / "bin" / "pixi")
    if not pixi or not Path(pixi).exists():
        pytest.skip("pixi not available; skipping Mojo route smoke test")
    ...
    convert_dsn_to_problem(dsn_path=dsn, out_json=problem, net_limit=2)
