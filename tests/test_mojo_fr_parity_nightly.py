...
def _run_check(*, fixture: Path, out_dir: Path, timeout_s: float) -> dict:
    ...
    proc = subprocess.run(
        ...
        capture_output=True,
        text=True,
        timeout=max(int(timeout_s) + 180, 600),
    )
...
@pytest.mark.slow
@pytest.mark.skipif(
    os.environ.get("RUN_NIGHTLY_PARITY") != "1",
    reason="set RUN_NIGHTLY_PARITY=1 to run nightly parity fixtures",
)
@pytest.mark.parametrize("fixture_rel", NIGHTLY_FIXTURES)
def test_mojo_matches_fr_baseline_nightly(fixture_rel: str, tmp_path: Path) -> None:
    ...
    out_dir = _repo_root() / "build" / "pytest_parity_baseline" / tmp_path.name / "nightly"
    out_dir.mkdir(parents=True, exist_ok=True)
    _run_check(fixture=fixture, out_dir=out_dir, timeout_s=420.0)
