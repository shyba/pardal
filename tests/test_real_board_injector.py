...
def test_injector_board_power_nets_first(injector_board_path, injector_placements):
    ...
    This follows the recommended workflow: route critical power nets
    manually or first, then autoroute remaining signals.
    """
    repl = REPL()
    ...
    gnd_result = repl.process_command("AUTOROUTE NET GND PREFER B.Cu")
    assert "OK:" in gnd_result or "routed" in gnd_result.lower()

    vcc_result = repl.process_command("AUTOROUTE NET +12V PREFER F.Cu")
    assert "OK:" in vcc_result or "routed" in vcc_result.lower()

    v5_result = repl.process_command("AUTOROUTE NET +5V PREFER F.Cu")
    assert "OK:" in v5_result or "routed" in v5_result.lower()

    # Route remaining nets
    remaining_result = repl.process_command("AUTOROUTE UNROUTED")
    end_time = time.time()

    routing_time = end_time - start_time

    # Check results
    assert "OK:" in remaining_result or "routed" in remaining_result.lower()
    assert routing_time < 60.0, f"Routing took {routing_time:.1f}s"

    # DRC check
    drc_result = repl.process_command("CHECK DRC")

    print(f"\n=== Incremental Routing Results ===")
    print(f"Routing time: {routing_time:.2f}s")
    print(f"GND: {gnd_result[:100]}")
    print(f"+12V: {vcc_result[:100]}")
    print(f"+5V: {v5_result[:100]}")
    print(f"Remaining: {remaining_result[:100]}")
    print(f"DRC: {drc_result[:200]}")
...
def test_injector_board_optimize_after_routing(injector_board_path, injector_placements):
    ...
    This tests the complete workflow: autoroute + optimize.
    """
    repl = REPL()
    ...
    # Autoroute all
    start_time = time.time()
    route_result = repl.process_command("AUTOROUTE ALL")
    mid_time = time.time()

    assert "OK:" in route_result or "routed" in route_result.lower()

    # Count vias before optimization
    vias_before = route_result.lower().count("via")

    # Optimize
    optimize_result = repl.process_command("OPTIMIZE ALL")
    end_time = time.time()

    total_time = end_time - start_time
    route_time = mid_time - start_time
    optimize_time = end_time - mid_time

    # Count vias after optimization
    vias_after = optimize_result.lower().count("via")

    # Check results
    assert "OK:" in optimize_result or "optimized" in optimize_result.lower()
    assert total_time < 70.0, f"Total time {total_time:.1f}s exceeded 70s"

    print(f"\n=== Autoroute + Optimize Results ===")
    print(f"Route time: {route_time:.2f}s")
    print(f"Optimize time: {optimize_time:.2f}s")
    print(f"Total time: {total_time:.2f}s")
    print(f"Route result: {route_result[:150]}")
    print(f"Optimize result: {optimize_result[:150]}")
