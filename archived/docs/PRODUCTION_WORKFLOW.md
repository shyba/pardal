...
If `kicad-cli` isn’t installed locally, you can run it via Docker:

```bash
docker run --rm -v "$PWD:/work" -w /work kicad/kicad:9.0.6-full kicad-cli pcb drc final.kicad_pcb
```
