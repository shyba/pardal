...
DEFAULT_CASES_JSON = PROJECT_ROOT / "parity_fixtures" / "fr_runtime_feature_cases.json"
...
def _run_suite(
    *,
    ...
    cmd = [
        ...
        "-m",
        "pcb_tool.tools.run_parity_suite",
        "--fixtures-json",
        str(fixtures_json),
        "--out-dir",
        str(suite_out_dir),
...
def _summarize_trace(trace_path: Path) -> Path:
    cmd = [
        str(PROJECT_ROOT / "venv" / "bin" / "python"),
        str(PROJECT_ROOT / "pcb_tool" / "tools" / "summarize_freerouting_trace.py"),
        "--in",
        str(trace_path),
    ]
